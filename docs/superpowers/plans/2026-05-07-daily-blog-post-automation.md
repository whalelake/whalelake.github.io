...
**Goal:** Generate three Jekyll blog posts every day for Current Affairs, AI, and Development, then commit them automatically from GitHub Actions.

**Architecture:** A small Python CLI creates deterministic Markdown posts under `_posts`. A scheduled GitHub Actions workflow runs the CLI daily in Asia/Seoul timing, commits only when files changed, and then the existing Pages workflow publishes the pushed commit.
...
- [x] Generate one post each for `Current Affairs`, `AI`, and `Development`.
